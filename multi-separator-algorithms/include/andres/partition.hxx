...
/// Returns true if the component containing element is a singleton
/// 
/// \param element index of element that should be checked 
///
template<class T>
inline bool 
Partition<T>::isSingleton(
    Index element
) {
    return ranks_[find(element)] == 0;
}
