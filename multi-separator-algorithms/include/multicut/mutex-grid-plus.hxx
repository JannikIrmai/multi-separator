...
public:
    ...
    GridMutexPlus(std::array<size_t, D> shape, std::vector<std::array<int, D>> connectivity, size_t separating_channel)
    :
        shape(shape),
        ...
        background()
    {
        ...
        mutexes.resize(n);
        // set the background to all zeros, i.e. all vertices are not assigned to the background
        background.resize(n, 0);
    }
    ...
    {    
        for (size_t counter = 0; counter < sorted_indices.size(); ++counter)
        {
            ...
            // check if the idx refers to an edge or to a vertex
            if (idx < n)  // idx refers to a pixel
            {
                // if the component containing idx is a singleton, add idx to the background
                if (partition.isSingleton(idx))
                    background[idx] = 1;   
            }
            else  // idx refers to an edge
            {
                idx -= n;
                ...
                if (!std::get<0>(e))
                    continue;
                size_t u = std::get<1>(e);
                size_t v = std::get<2>(e);

                // continue if either vertex is in the background
                if ((background[u] == 1) || (background[v] == 1))
                    continue;

                // merge the components or add a mutex constraint
                if (idx / n < separating_channel)
                    merge(u, v);
                else
                    add_mutex(u, v);
            }
